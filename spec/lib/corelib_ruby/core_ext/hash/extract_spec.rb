  ...
  describe "#extract" do
    ...
    context 'yields' do
      it "when key is missing and no default is given" do
        hash = { one: 1, two: 2 }
        expect { |arg| hash.extract(:three, &arg) }.to yield_with_args(:three)
      end
      it "when key is missing and default is given" do
        hash = { one: 1, two: 2 }
        expect { |arg| hash.extract(:three, 3, &arg) }.to yield_with_args(:three)
      end
    end
    ...
    context "raises" do
      it 'ArgumentError when no arguments given' do
        hash = { one: 1, two: 2 }
        expect { hash.extract }.to raise_exception(ArgumentError)
      end
      it 'ArgumentError when 3 or more arugments are given' do
        hash = { one: 1, two: 2 }
        expect { hash.extract(1, 2, 3) }.to raise_exception(ArgumentError)
      end
      it 'KeyError when key is missing and no default is given' do
        hash = { one: 1, two: 2 }
        expect { hash.extract(:three) }.to raise_exception(KeyError)
      end
    end
  end
end
